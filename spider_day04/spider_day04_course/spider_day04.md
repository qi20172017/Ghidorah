...
- **达内code课程方向案例**
  ...
  【知识点】
    1) auth参数 - ('用户名','密码')
    2) 字符串方法: 
       a> string.endswith('xxx')
       b> string.startswith('xxx')
       c> string.split('xxx')
       d> 'xxx'.join([])
       e> string.replace('原字符串','新字符串')
       f> 切片: 正向切 反向切
  ```
...
```python
【1】实现有道翻译 : requests.post(url=url,headers=headers,data=data)
     url: F12抓到的 Headers -> General -  Request URL
